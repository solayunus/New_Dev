...
//admin/employee sigin 

const signin = (request, response) => {
    const { email, upassword } = request.body;
    const queryString = 'SELECT * FROM users WHERE email = $1';
    helper.pool.query(queryString, [email], (error, results) => {
        if (error) {
            // throw error;
...
const getUserById = (request, response) => {
    ...
    helper.pool.query('SELECT * FROM users WHERE id = $1', [id], (error, results) => {
        if (error) {
            //     throw error;
        }
...
const createUser = (request, response) => {
    ...
    let queryString = 'INSERT INTO users (firstname, lastname, email, upassword, gender, jobrole, department, address) VALUES ($1, $2, $3, $4, $5, $6, $7, $8);';
    helper.pool.query(queryString, [firstname, lastname, email, upassword, gender, jobrole, department, address], (error, results) => {
        if (error) {
        ...
        response.status(200).json({
            status: "success",
            data: {
                message: "Users account successfully created",
                token: results,
...
const updateUser = (request, response) => {
    ...
    helper.pool.query(queryString, [name, email, id],
        (error, results) => {
            if (error) {
                throw error;
            }
...
const deleteUser = (request, response) => {
    ...
    helper.pool.query('DELETE FROM users WHERE id = $1', [id], (error, results) => {
        if (error) {
            throw error
        }
